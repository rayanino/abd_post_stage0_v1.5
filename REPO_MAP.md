...
| `1_normalization/` | HTML → structured JSONL | High (spec validated, gold exists) | `NORMALIZATION_SPEC_v0.2.md`, `SHAMELA_HTML_REFERENCE.md`, `CORPUS_SURVEY_REPORT.md`, `gold_samples/` |
| `2_structure_discovery/` | Detect divisions, build passage boundaries | Draft (least mature) | `STRUCTURE_SPEC.md`, `structural_patterns.yaml`, 3 corpus surveys |
| `3_atomization/` | Break passages into atoms | Draft (rules mature, automation TBD) | `ATOMIZATION_SPEC.md` (has schema drift — see ZOOM_BRIEF) |
| `4_excerpting/` | Group atoms into excerpts, assign to taxonomy | Draft (most complex) | `EXCERPTING_SPEC.md`, `EXCERPT_DEFINITION.md` (~1200 lines, comprehensive) |
...
|------|-------|----------------|
| `precision/00_BINDING_DECISIONS_v0.3.15.md` | 358 | Atom boundaries, excerpt rules, topic scope guard, core duplication, headings |
| `precision/checklists_v0.3.md` | 178 | ATOM.*, EXC.*, PLACE.*, REL.* checklist items |
| `precision/extraction_protocol_v2.md` | 15 | Checkpoint sequence (CP1–CP6) |
| `precision/project_glossary.md` | 519 | Authoritative definitions for all terms |

**Rule:** Stage 3/4 specs say "rules are NOT restated here — canonical source is binding decisions + checklists." When in doubt, precision/ overrides stage specs.

### Gold Baselines (Proven Ground Truth)

```
gold_baselines/jawahir_al_balagha/
├── passage1_v0.3.13/   (59 atoms, 47 excerpts, 21 decisions)  — pages 19–25
...
| `tools/normalize_shamela.py` | 595 | 1 | HTML → pages.jsonl (deterministic) |
| `tools/extract_clean_input.py` | 234 | 3 (CP1) | Extract clean text from HTML for atomization |
| `tools/validate_gold.py` | — | QA | Validate gold baselines against schema |
| `tools/render_excerpts_md.py` | — | QA | Render excerpts as readable Markdown |
| `tools/scaffold_passage.py` | — | Util | Create passage directory structure |
| `tools/pipeline_gold.py` | — | Util | Run full gold pipeline |
| `tools/build_baseline_manifest.py` | — | Util | Generate baseline manifests |
| `tools/run_all_validations.py` | — | QA | Run all validation checks |
...
112 tests: 92 intake + 16 enrich + 4 interactive multi-science.

---

## Known Schema Drift

The ZOOM_BRIEF files in stages 3 and 4 document **schema drift** between the stage specs and the actual gold data. The gold data + `schemas/gold_standard_schema_v0.3.3.json` + `precision/` are authoritative. Stage specs that contradict them need updating. Key drifts:
