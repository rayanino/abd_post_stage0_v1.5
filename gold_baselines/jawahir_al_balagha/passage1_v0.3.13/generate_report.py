...
def main():
    ...
    for nid, excs in node_map.items():
        ...
        for e in excs:
            ...
            title = e.get('excerpt_title')
            if title:
                L.append(f"    +-- {e['excerpt_id']} — {title} [{e['excerpt_kind']}{rs}] core={nc}, ctx={nctx}, layer={e['source_layer']}")
            else:
                L.append(f"    +-- {e['excerpt_id']} [{e['excerpt_kind']}{rs}] core={nc}, ctx={nctx}, layer={e['source_layer']}")
