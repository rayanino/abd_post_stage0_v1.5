...
#
# This validator is a historical snapshot bundled with this baseline for
# traceability only.
#
# Do NOT use it for current validation behavior or automation specs.
# Canonical validator: ABD/tools/validate_gold.py
# Canonical validation entrypoint (repo root):
#   (cd ABD && python tools/run_all_validations.py)
# ============================================================

"""
Gold Standard Validator v0.3.9
Validates atoms, excerpts, exclusions, taxonomy_changes, and decision traceability
against gold_standard_schema_v0.3.1.json with comprehensive structural invariants.
...
BANNER = "Gold Standard Validator v0.3.9"
...
def validate_excerpts(excerpts, atom_map, report, allow_unknown_vocab=False,
                     allow_external_relations=False):
    ...
    for line_num, exc in excerpts:
        ...
        # ── Cross-science ──
        cs = exc.get("cross_science_context", False) or              exc.get("rhetorical_treatment_of_cross_science", False)
        rs = exc.get("related_science")
        if cs and rs is None:
            report.error(f"{eid}: cross-science flag true but related_science null")
        if not cs and rs is not None:
            report.warn(f"{eid}: related_science set but no cross-science flag")
...
def validate_exclusions(exclusions, atom_map, report):
...
def validate_decisions(decisions_path, excerpt_ids, checklists_path, report):
    ...
    for line_num, rec in records:
        ...
        if rt == "excerpt_decision":
            ...
            # Checklist version
            cv = rec.get("checklist_version", "")
            if not cv:
                report.warn(f"[decisions] '{eid}': missing checklist_version")
...
def validate_boundary_reasoning_blocks(excerpts, report):
...
def validate_taxonomy_changes(tc_records, schema, report, excerpt_ids=None,
                              excerpt_records=None, allow_external_excerpts=False):
...
def main():
    ...
    if exclusions:
        validate_exclusions(exclusions, atom_map, report)
    ...
    if not args.skip_traceability:
        ...
        if args.decisions and excerpts:
            validate_decisions(args.decisions, set(exc_ids.keys()),
                             args.checklists, report)
        ...
        if excerpts:
            validate_boundary_reasoning_blocks(excerpts, report)
