...
When an excerpt relies on concepts from another linguistic science (e.g., a بلاغة excerpt requiring understanding of صرف concepts), `cross_science_context` is true and `related_science` identifies which science. The companion field `rhetorical_treatment_of_cross_science` distinguishes two cases: the author treating the other science's concepts technically (reader must understand them) versus rhetorically (as illustrations, no deep understanding required).
