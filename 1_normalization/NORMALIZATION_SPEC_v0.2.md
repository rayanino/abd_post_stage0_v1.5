...
In both cases, the compound key `(volume, page_number_int)` is guaranteed unique.
...
**Unique key:** The compound key `(book_id, volume, page_number_int)` is guaranteed unique across all records in a JSONL file.
...
**Rule V1:** Asterisk-wrapped verses (`* text *`) are detected and the asterisks stripped, leaving the verse text.

**Rule V2:** The `has_verse` flag is set to `true` when the page contains either asterisk-wrapped verses or the hemistich separator character `…` (U+2026 HORIZONTAL ELLIPSIS).
...
### 4.12 Cross-Reference Validation
...
6. **Content tables:** Rare comparison/conjugation tables (~206 across 189,676 pages) in 3 books. Extracted as tab-separated text.
